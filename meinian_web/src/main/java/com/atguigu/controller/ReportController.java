...
public class ReportController {
    ...
    /**
     * 不是ajax请求，不用返回Result对象了，直接写出流数据（那个要导出的excel文件）即可
     * @param request
     * @param response
     */
    ...
    public void exportBusinessReport(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            //2.获取模板文件（绝对路径）
            // 前面的request.getSession().getServletContext().getRealPath("template")
            // 相当于是E:\workspace\sgghouduan-workspace\sggMay\part05-gaoji-meinianlvyou\meinian-parent\meinian_web\target\meinian_web-1.0-SNAPSHOT\template
            // 关于文件分隔符linux是左斜杠/，win是右斜杠\，可以用File.separator来让代码在不同系统上自动生成文件分隔符
